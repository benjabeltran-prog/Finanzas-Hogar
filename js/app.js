...
async function loadDashboard() {
  ...
  const labels = (allSummaries || []).map((s) => `${MONTH_NAMES[s.month - 1].slice(0,3)} ${s.year}`);
  const values = (allSummaries || []).map((s) => Number(s.ahorro));

  const ctx = document.getElementById("chart-savings").getContext("2d");
  if (savingsChart) savingsChart.destroy();
  savingsChart = new Chart(ctx, {
    type: "bar",
    data: { labels, datasets: [{ label: "Ahorro", data: values, backgroundColor: values.map(v => v >= 0 ? "#34d399" : "#f87171") }] },
    options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { ticks: { color: "#8792a8" } }, x: { ticks: { color: "#8792a8" } } } },
  });
}
